...
//TIME
		...
		void uniformTimeSearch(long source, long dest){
			...
			auto srcItr = intersections.find(source);
			auto destItr = intersections.find(dest);
			current.estimate = HaversineDistance(srcItr->second.lat, srcItr->second.lon, destItr->second.lat, destItr->second.lon)/65;
			...
			do{
				...
				for(auto elem : intersections.find(current.path.back())->second.connections){
					...
					if(visited.find(elem.endID) == visited.end()){
						...
						buff.estimate = buff.totalTime + HaversineDistance(elem.endLat, elem.endLon, destItr->second.lat, destItr->second.lon)/65;
